...
void * producer(void *threadid){
	...
	for(i = 0; i < countElems; i++){ // Produce as many elements as indicated
   	...
   	long tid = pthread_self(); // Need to include this line, else there is no output
   	return NULL;
}
...
int main (int argc, const char * argv[] ){
	...
	semName = argv[2];
	maxSize = atoi(argv[3]);
	countElems = atoi(argv[4]);

	if(maxSize < 0 || id < 0 || countElems < 0){ // Any argument is negative the file is invalid
		argsNotValid();
		return -1;
	} 
